            ...
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/50">
                {['Name', 'Category', 'Price', 'Description', 'Status', 'Actions'].map((header) => (
                  <th key={header} className="px-6 py-4 text-left text-xs font-['Gilroy_Bold'] text-gray-400 uppercase tracking-widest">
                    {header}
                  </th>
                ))}
              </tr>
                ...
                <tr key={item._id} className="border-b border-gray-50 last:border-0 hover:bg-orange-50/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-['Gilroy_Bold'] text-gray-900">{item.name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="px-3 py-1 bg-orange-50 text-orange-600 rounded-full text-xs font-['Gilroy_Bold']">
                      {item.category || 'Uncategorized'}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-['Gilroy_Medium'] text-gray-700">LKR {Number(item.price || 0).toFixed(2)}</td>
                  <td className="px-6 py-4 font-['Gilroy_Medium'] text-gray-500 max-w-xs truncate">
                    {item.description || 'No description'}
                  </td>
                  <td className="px-6 py-4">
                    {item.isAvailable !== false ? (
                      <span className="flex items-center gap-1 text-green-600 text-xs font-['Gilroy_Bold']">
                        <CheckCircle size={14} /> Available
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-red-500 text-xs font-['Gilroy_Bold']">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <form id="menuForm" onSubmit={handleSave} className="space-y-4 overflow-y-auto flex-1 p-8">
              ...
              <div>
                <label className="block text-xs font-['Gilroy_Bold'] text-gray-500 uppercase tracking-wider mb-3">Image</label>
                ...
                <div className="flex gap-2 mb-3">
                  <button
                    ...
                    className="flex items-center gap-2 flex-1 px-4 py-3 bg-orange-50 border-2 border-orange-200 rounded-xl text-orange-600 font-['Gilroy_Bold'] text-sm hover:bg-orange-100 transition-all"
                  >
                    <Upload size={16} />
                    Browse Files
                  </button>
                  {form.image && (
                    <button
                      ...
                      className="px-4 py-3 bg-red-50 border-2 border-red-200 rounded-xl text-red-600 font-['Gilroy_Bold'] text-sm hover:bg-red-100 transition-all"
                    >
                      Remove
                    </button>
                  )}
                ...
                <div className="space-y-2">
                  <p className="text-[10px] text-gray-500 font-['Gilroy_Medium']">OR paste image URL:</p>
                  <input
                    type="text"
                    placeholder="https://example.com/image.jpg"
                    value={form.image}
                    onChange={(e) => setForm((prev) => ({ ...prev, image: e.target.value }))}
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent rounded-xl focus:border-orange-500 focus:bg-white outline-none font-['Gilroy_Medium'] transition-all text-sm"
                  />
            ...
            <div className="flex gap-3 p-8 border-t border-gray-100 bg-gray-50 shrink-0">
              <button
                ...
                className="flex-1 py-3 rounded-2xl border-2 border-gray-100 font-['Gilroy_Bold'] text-gray-500 hover:bg-gray-100 transition-all"
              >
                Cancel
              </button>
              <button
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 p-8 text-center">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-2xl border-2 border-gray-100 font-['Gilroy_Bold'] text-gray-500 hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
                onClick={() => handleDelete(deleteId)}
                className="flex-1 py-3 bg-red-500 text-white rounded-2xl font-['Gilroy_Bold'] hover:bg-red-600"
              >
                Delete
              </button>
            </div>
