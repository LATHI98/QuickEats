    ...
    >
      ...
      <div className="space-y-4 p-6">
        ...
        <div className="flex items-center gap-4 mb-5">
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onRate('ratings'); }}
            className="flex items-center gap-1.5 font-['Gilroy_Bold'] text-amber-600 hover:text-amber-700 transition-colors text-xs"
          >
            <Star size={12} strokeWidth={2.5} /> Rate
          </button>
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onRate('reviews'); }}
            className="flex items-center gap-1.5 font-['Gilroy_Bold'] text-orange-600 hover:text-orange-700 transition-colors text-xs"
          >
            <MessageCircle size={12} strokeWidth={2.5} /> Review
          </button>
        </div>
...
const StudentDashboard = () => {
  ...
  // Review Modal State
  const [reviewModal, setReviewModal] = useState({ isOpen: false, targetId: null, targetName: '', type: 'canteen', mode: 'ratings' });
        ...
        <section className="relative overflow-hidden rounded-[40px] border border-gray-100 bg-gray-900 text-white shadow-2xl shadow-orange-200/20 group">
          ...
          <div className="relative z-10 grid gap-6 px-10 py-6 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-1">
              <StatCard icon={TimerReset} label="Active Orders" value={activeOrders.length} tone="text-white" dark />
              <StatCard icon={CheckCircle2} label="Ready for pickup" value={readyOrders} tone="text-white" dark />
            </div>
          </div>
        </section>
        ...
        {latestCompletedOrder && (
          <section>
            <motion.div
              ...
              className="flex flex-col items-center justify-between gap-6 rounded-[40px] border border-orange-100 bg-white p-8 shadow-xl shadow-orange-200/20 md:flex-row"
            >
              <div className="flex items-center gap-6">
                <div className="flex h-20 w-20 items-center justify-center rounded-[32px] bg-orange-50 text-orange-600 shadow-inner">
                  <UtensilsCrossed size={36} />
                </div>
                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-gray-400">Order again?</p>
                  <h2 className="mt-1 text-2xl font-['Gilroy_Heavy'] text-gray-900">Your last meal was great!</h2>
                  <p className="mt-1 text-sm text-gray-500">
                    {latestCompletedOrder.canteen?.name} · {formatMoney(latestCompletedOrder.totalPrice)}
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-[24px] bg-gray-900 px-8 py-4 font-['Gilroy_Heavy'] text-white shadow-xl transition-all hover:bg-black hover:scale-105 active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {canteens.slice(0, 6).map((canteen) => (
                <CanteenCard
                  ...
                  onRate={(mode) => setViewReviewsModal({ isOpen: true, targetId: canteen._id, targetName: canteen.name, mode })}
                />
              ))}
            </div>
          )}
