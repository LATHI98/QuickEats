  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-['Gilroy_Bold'] text-gray-900">Staff Management</h1>
          <p className="text-gray-400 font-['Gilroy_Medium'] mt-1">Manage canteen staff, roles and contact info.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-300">
            <Users size={48} />
            <p className="mt-4 font-['Gilroy_Bold'] text-gray-400">No staff yet — add one!</p>
          </div>
        ) : (
                ...
                <tr key={s._id} className="border-b border-gray-50 last:border-0 hover:bg-orange-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-orange-50 rounded-xl flex items-center justify-center text-orange-600 font-['Gilroy_Heavy'] text-sm">
                        {s.name?.[0]?.toUpperCase() || '?'}
                      </div>
                      <span className="font-['Gilroy_Bold'] text-gray-900">{s.name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-['Gilroy_Bold'] capitalize ${roleColor(s.role)}`}>
                      <Briefcase size={10} className="inline mr-1" />{s.role}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="flex items-center gap-1 text-gray-500 font-['Gilroy_Medium']">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md mx-4 p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-['Gilroy_Bold'] text-gray-900">{editing ? 'Edit Staff' : 'Add Staff'}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)}
                  className="flex-1 py-3 rounded-2xl border-2 border-gray-100 font-['Gilroy_Bold'] text-gray-500 hover:bg-gray-50">Cancel</button>
                <button type="submit" disabled={saving}
                  className="flex-1 py-3 bg-orange-600 text-white rounded-2xl font-['Gilroy_Bold'] hover:bg-orange-700 disabled:opacity-50">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm mx-4 p-8 text-center">
            ...
            <h3 className="text-lg font-['Gilroy_Bold'] text-gray-900 mb-2">Remove Staff?</h3>
            <p className="text-gray-400 font-['Gilroy_Medium'] text-sm mb-6">This action cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="flex-1 py-3 rounded-2xl border-2 border-gray-100 font-['Gilroy_Bold'] text-gray-500">Cancel</button>
              <button onClick={() => handleDelete(deleteId)} className="flex-1 py-3 bg-red-500 text-white rounded-2xl font-['Gilroy_Bold'] hover:bg-red-600">Remove</button>
            </div>
          </div>
        </div>
      )}
