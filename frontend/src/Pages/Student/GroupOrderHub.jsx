    ...
    if (!session || isSwitching) {
        return (
            <div className="max-w-2xl mx-auto py-8 px-4">
                <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Group Ordering</h1>
                        ...
                        {isCreating ? (
                            <form onSubmit={handleCreate} className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    <button type="button" onClick={() => setIsCreating(false)} className="px-4 py-2 border rounded-xl text-sm font-medium flex-1">Cancel</button>
                                    <button type="submit" disabled={actionLoading} className="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm font-medium flex-1 disabled:opacity-50">Start</button>
                                </div>
                            </form>
                        ) : (
                            <button
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                        ...
                        {isJoining ? (
                            <form onSubmit={handleJoin} className="space-y-4">
                                ...
                                <div className="flex gap-2">
                                    <button type="button" onClick={() => setIsJoining(false)} className="px-4 py-2 border rounded-xl text-sm font-medium flex-1">Cancel</button>
                                    <button type="submit" disabled={actionLoading} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium flex-1 disabled:opacity-50">Join</button>
                                </div>
                            </form>
                        ) : (
                            <button
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-gray-900 flex items-center gap-3">
                        {session.name}
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${session.status === 'locked' ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>
                ...
                <div className="flex gap-2">
                    ...
                    {isCreator && session.status === 'open' && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 border border-red-200 text-red-600 hover:bg-red-50 rounded-xl text-sm font-medium transition-colors"
                        >
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-xl text-sm font-medium transition-colors"
                    >
                    ...
                    <div className="flex-1 text-center md:text-left">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div>
                                ...
                                <button
                                    onClick={copyCode}
                                    className="px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors text-gray-600 text-sm font-medium flex items-center justify-center md:justify-start gap-2 mx-auto md:mx-0"
                                >
                ...
                <div className={`rounded-2xl p-6 border ${session.status === 'locked' ? 'bg-gray-50 border-gray-100 opacity-75' : 'bg-indigo-50 border-indigo-100'}`}>
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white py-3 rounded-xl font-extrabold hover:bg-indigo-700 transition-colors disabled:opacity-50"
                        >
                ...
                <div className={`rounded-2xl p-6 border ${session.status === 'locked' ? 'bg-green-50 border-green-200' : 'bg-orange-50 border-orange-100'}`}>
                    ...
                    {isCreator && session.status === 'open' && (
                        <button
                            onClick={handleLock}
                            disabled={actionLoading}
                            className="w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl font-extrabold transition-colors"
                        >
                    ...
                    {session.status === 'locked' && (
                        <button
                            onClick={() => navigate('/dashboard/cart')}
                            className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-extrabold transition-colors"
                        >
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
                        ...
                        <form onSubmit={handleSaveEdit} className="space-y-4">
                            ...
                            <div className="pt-2">
                                <button type="submit" disabled={actionLoading} className="w-full py-2.5 bg-gray-900 text-white rounded-xl text-sm font-extrabold">
                                    Save Changes
                                </button>
                            </div>
                        </form>
